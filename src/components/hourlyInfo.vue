...
import { filterData } from '../components/componentDependencies/forecastGetData.js'
...
// console.log(props.recievedData.value.value)
const idNumber = props.id
const Data = reactive({})
const time = ref('')
const temperature = ref('')
const rainIntensity = ref('')
const snowIntensity = ref('')
...
const weatherIcon = reactive({
    ...
    getInfoForWeatherIcon(Data) {
        ...
        this.snowIntensity = Data?.snowIntensity
    },
    setStates(){
        this.setIsCloudy()
        this.setIsSnowy()
...
watch(Data,(value)=>{
    const newData = new filterData({},idNumber,[])


    finalData.value = newData.getLoadedData(value.value)
    weatherIcon.getInfoForWeatherIcon(finalData.value)
    weatherIcon.setStates();
    time.value = finalData.value.time
    temperature.value = finalData.value.temperature
    cloudCover.value = finalData.value.cloudCover
    rainIntensity.value = finalData.value.rainIntensity
    snowIntensity.value = finalData.value.snowIntensity
    let timeInArray = time.value.split('T')
    timeToDisplay.value = timeInArray[1].split(':',1)[0]
 

})
Data.value = props.recievedData.value
...
<template>
    <div>
        <div class="time font-semibold text-lg mb-7">
            {{ `${timeToDisplay}h` }}
        </div>
        <div class="svg mb-7">
            <svg v-if="weatherIcon.isRainy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-10 h-10 fill-slate-700 rainy">
                        ...
                        <svg v-if="weatherIcon.isSunny" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-12 h-12 p-2 text-yellow-500 fill-current sunny">
                        ...
                        <svg v-if="weatherIcon.isCloudy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-10 h-10 fill-slate-700 cloudy">
                        ...
                        <svg v-if="weatherIcon.isPartlyCloudy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-10 h-10 fill-slate-700 partlyCloudy">
                        ...
                        <svg v-if="weatherIcon.isSnowy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-10 h-10 fill-slate-700 snowy">
        ...
        <div class="temperature font-semibold text-lg mr-3">
            {{`${temperature}°` }}
        </div>
    </div>
</template>